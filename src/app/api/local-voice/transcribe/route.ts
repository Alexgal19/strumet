...
export async function POST(request: Request) {
  try {
    ...
    return await exclusive(async () => withTemp(async dir => {
      ...
      await run(process.env.LOCAL_VOICE_FFMPEG_PATH || 'ffmpeg', ['-v', 'error', '-y', '-protocol_whitelist', 'file,pipe', '-f', format, '-i', audio, '-t', '31', '-ar', '16000', '-ac', '1', '-c:a', 'pcm_s16le', wav], 20000);
      await run(process.env.LOCAL_VOICE_WHISPER_PATH || 'whisper-cli', ['-m', model, '-f', wav, '-l', 'pl', '-otxt', '-of', output, '-nt'], 90000);
