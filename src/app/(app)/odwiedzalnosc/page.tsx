...
import { Loader2, ArrowLeft, ArrowRight, User, TrendingDown, CalendarDays, Building, Search, UserX, Copy } from 'lucide-react';
...
export default function AttendancePage() {
  ...
  const handleCopyAbsenceList = () => {
    if (absencesByEmployee.length === 0) return;

    const listText = absencesByEmployee
      .map(({ employee, dates }) => `${employee.fullName}: ${dates.join(', ')}`)
      .join('\n');

    navigator.clipboard.writeText(listText).then(() => {
      toast({
        title: 'Skopiowano!',
        description: 'Lista nieobecności została skopiowana do schowka.',
      });
    }).catch(err => {
      ...
      <Card className="flex-grow flex flex-col">
       ...
       <Dialog open={isAbsenceListOpen} onOpenChange={setIsAbsenceListOpen}>
          <DialogContent className="sm:max-w-md">
              ...
              {absencesByEmployee.length > 0 && (
                <DialogFooter>
                    <Button onClick={handleCopyAbsenceList}>
