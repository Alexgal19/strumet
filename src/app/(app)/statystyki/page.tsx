...
const ReportTab = forwardRef<unknown, {}>((_, ref) => {
  const { employees, config, isAdmin, handleSaveEmployee } = useAppContext();
  ...
  const { stats, departmentData, nationalityData, jobTitleData } =
    useMemo(() => {
      ...
      activeEmployees.forEach((employee) => {
        ...
        if (employee.jobTitle)
          jobCounts[employee.jobTitle] =
            (jobCounts[employee.jobTitle] || 0) + 1;
      });
              ...
              <AccordionTrigger className="hover:no-underline text-sm">
                <div className="flex justify-between w-full pr-2">
                  ...
                  <span className="text-muted-foreground text-xs">
                    {Object.values(jobTitles).flat().length} os.
                  </span>
                </div>
              </AccordionTrigger>
              <AccordionContent>
    ...
    <div className="flex flex-col space-y-6 flex-grow">
      ...
      {employees.length === 0 ? (
        <div className="text-center text-muted-foreground py-10">
          Brak danych do wyświetlenia statystyk. Dodaj pracowników, aby zobaczyć
          analizę.
        </div>
      ) : (
        <>
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Liczba unikalnych stanowisk w firmie
                </p>
              </CardContent>
            </Card>
          </div>
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="lg:col-span-2">
              {renderPieChart(
                jobTitleData,
                "Rozkład wg Stanowisk",
                "Liczba pracowników na poszczególnych stanowiskach.",
                "jobTitle",
              )}
            </div>
          </div>
      ...
      <Dialog open={isStatDialogOpen} onOpenChange={setIsStatDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          ...
          <ScrollArea className="max-h-[60vh] my-4">
            <div className="space-y-1 pr-4">{renderDialogContent()}</div>
          </ScrollArea>
        </DialogContent>
      </Dialog>
...
const DiffBadge = ({ diff }: { diff: number }) => (
  <Badge
    ...
    className={cn(diff >= 0 && "bg-green-500 hover:bg-green-600")}
  >
    {diff > 0 ? `+${diff}` : diff}
  </Badge>
);

const PeriodChangeCard = ({
  title,
  data,
}: {
  title: string;
  data: { name: string; from: number; to: number; diff: number }[];
}) => (
  <Card>
    <CardHeader>
...
const SingleDayReportCard = ({
  title,
  data,
}: {
  title: string;
  data: { name: string; to: number }[];
}) => (
  <Card>
    <CardHeader>
      <CardTitle className="text-base">{title}</CardTitle>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-lg">
              Zmiany w danych pracowników
            </CardTitle>
            <CardDescription>
              {changes.length} zarejestrowanych zmian
            </CardDescription>
          </div>
        </div>
      </CardHeader>
        ...
        <Accordion type="multiple" defaultValue={Object.keys(groupedChanges)}>
          {Object.entries(groupedChanges).map(([type, items]) => (
            <AccordionItem value={type} key={type}>
              <AccordionTrigger>
                Zmiany działów ({(items as any[]).length})
              </AccordionTrigger>
              <AccordionContent>
...
const HiresAndFiresTab = () => {
  ...
  const handleManualArchive = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || "Wystąpił błąd po stronie serwera.",
      });
    } finally {
  ...
  const generateReport = async () => {
    if (!date || !date.from) {
      toast({
        variant: "destructive",
        title: "Błąd",
        description: "Proszę wybrać datę lub zakres dat.",
      });
      return;
    }
  ...
  const hasEvents =
    report &&
    ((report.newHires && report.newHires.length > 0) ||
      (report.terminated && report.terminated.length > 0) ||
      (report.fieldChanges && report.fieldChanges.length > 0));
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex justify-between items-start">
            <div>
              ...
              <CardDescription>
                Porównaj stan zatrudnienia między dwoma dniami lub zobacz stan
                na jeden dzień.
              </CardDescription>
            </div>
            {isAdmin && (
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Spowoduje to utworzenie nowego archiwum na dzień
                      dzisiejszy. Jeśli archiwum na dziś już istnieje, zostanie
                      ono nadpisane.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>Anuluj</AlertDialogCancel>
                    <AlertDialogAction onClick={handleManualArchive}>
                      Kontynuuj
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
      ...
      {isLoading && !report && (
        <div className="flex justify-center items-center py-10">
          ...
          <p className="ml-4 text-muted-foreground">
            Generowanie raportu, to może potrwać chwilę...
          </p>
        </div>
      )}
        ...
        <div className="space-y-6 animate-fade-in">
          <Card className="bg-muted/30">
            <CardHeader>
              ...
              <CardDescription>
                Porównanie stanu z dnia{" "}
                <span className="font-bold">{report.start.date}</span> i{" "}
                <span className="font-bold">{report.end.date}</span>.
              </CardDescription>
            </CardHeader>
              ...
              <div className="p-4 rounded-lg border bg-background text-center">
                ...
                <div className="flex items-center justify-center gap-4 mt-2">
                  <span
                    className={cn(
                      "text-3xl font-bold",
                      report.diff >= 0 ? "text-green-500" : "text-destructive",
                    )}
                  >
                    {report.diff > 0 ? "+" : ""}
                    {report.diff}
              ...
              <div className="p-4 rounded-lg border bg-background text-center">
                ...
                <div className="flex items-center justify-center gap-4 mt-2">
                  <span className="text-2xl font-bold text-green-500">
                    +{report.newHires.length}
                  </span>
                  <span className="text-2xl font-bold text-destructive">
                    -{report.terminated.length}
                  </span>
                </div>
              </div>
            </CardContent>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <PeriodChangeCard
              title="Zmiany w działach"
              data={report.deptChanges}
            />
            <PeriodChangeCard
              title="Zmiany na stanowiskach"
              data={report.jobTitleChanges}
            />
            <PeriodChangeCard
              title="Zmiany narodowości"
              data={report.nationalityChanges}
            />
          </div>
        </div>
      )}
      ...
      {report && hasEvents && (
        <div className="space-y-6 animate-fade-in">
          {report.isRange ? (
            ...
            <Card className="bg-muted/30">
              <CardHeader>
                ...
                <CardDescription>
                  Stan zatrudnienia i zdarzenia w wybranym dniu.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 rounded-lg border bg-background text-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <EmployeeChangeList
              title={
                report.isRange ? "Nowo zatrudnieni" : "Zatrudnieni w tym dniu"
              }
      ...
      {report && !report.isRange && (
        <div className="space-y-6 animate-fade-in">
          <h3 className="text-lg font-semibold mt-8">
            Statystyki na dzień {report.date}
          </h3>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <SingleDayReportCard
              title="Liczba pracowników w działach"
              data={report.deptChanges}
            />
            <SingleDayReportCard
              title="Liczba pracowników na stanowiskach"
              data={report.jobTitleChanges}
            />
            <SingleDayReportCard
              title="Liczba pracowników wg narodowości"
              data={report.nationalityChanges}
            />
          </div>
        </div>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={setNewOrderDepartment}
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      value={newOrderJobTitle}
                      onValueChange={setNewOrderJobTitle}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Wybierz stanowisko" />
                      </SelectTrigger>
                      <SelectContent>
                        {config.jobTitles.map((j) => (
            ...
            <TabsContent value="replacement">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Wypełnij formularz, aby utworzyć zamówienie na zastępstwo.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-1">
                  ...
                  <Button
                    onClick={() => handleAddNewOrder("replacement")}
                    className="w-full"
                  >
                    <PlusCircle className="mr-2 h-4 w-4" />
                    Dodaj zastępstwo
      ...
      <div className={isAdmin ? "lg:col-span-2" : "lg:col-span-3"}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Lista aktualnych zapotrzebowań na personel.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {Object.keys(groupedOrders).length > 0 ? (
              <Accordion type="multiple" className="w-full">
                {Object.entries(groupedOrders).map(([dept, orderList]) => {
                  ...
                  const hasReplacementOrders = orderList.some(
                    (o) => o.type === "replacement",
                  );
                      ...
                      <AccordionTrigger>
                        <div className="flex justify-between w-full pr-4 items-center">
                          <div className="flex items-center gap-2">
                            ...
                            {hasNewOrders && (
                              <Badge variant="outline">Nowe</Badge>
                            )}
                            {hasReplacementOrders && (
                              <Badge
                                variant="secondary"
                                className="border-orange-500/80 text-orange-800"
                              >
                                Zastępstwo
                              </Badge>
                            )}
                          ...
                          <span className="text-muted-foreground text-sm font-normal">
                            {orderList.reduce((sum, o) => sum + o.quantity, 0)}{" "}
                            os.
                          </span>
                        </div>
                      </AccordionTrigger>
